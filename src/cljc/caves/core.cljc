...
(defn update-state [{{:keys [debug] :as settings} :settings, :as state}]
  (cond-> state
    (and (not (:pause debug))
         (<= (/ 1000 (:ups settings)) (- (quil/millis) (:last-update state 0))))
...
(defn setup! []
  (let [state (update-state default-state)]
  ...
  (let [settings (get-in default-state [:settings])]
    (quil/defsketch caves
      ...
      :middleware (cond-> [quil.mw/fun-mode
                           ...
                           (mw/mw! :draw #(mw/show-state! (dissoc % :slice :debug) (apply quil/create-font (:font settings))))]
